...
## 6b. Anbindung über Siemens LOGO! 8 (Modbus TCP, gleiches Netzwerk)

Wenn der Sensor an eine **Siemens LOGO! 8** angeschlossen ist (die eine
eigene IP-Adresse im selben Netzwerk wie der Pi hat), kann der Pi die
LOGO! 8 über **Modbus TCP** abfragen — das ist der Standard-Weg, den die
LOGO! 8 ohne Zusatzhardware unterstützt.

**Wichtiger Hinweis vorab:** Die LOGO! 8 ist eine SPS mit einem Scan-Zyklus
und einer Modbus-Antwortzeit im Bereich von einigen Millisekunden pro
Registerabfrage. Das genügt gut für ein Auslöse-Signal ("Ereignis erkannt")
oder einen einzelnen langsamen Messwert — aber **nicht**, um die feinen
512 Messpunkte eines schnellen Aufprall-Ereignisses in Echtzeit zu
übertragen (das würde Sekunden statt Millisekunden dauern). Falls die
eigentliche 512-Punkte-Aufnahme von einem anderen, schnelleren Gerät stammt
(vermutlich dem, das die ursprünglichen .txt-Dateien erzeugt hat), sollte
dieses Gerät direkt mit dem Pi sprechen (siehe Abschnitt 6); die LOGO! 8
kann trotzdem parallel als Auslöse-Quelle dienen.

**Einrichtung:**

1. In **LOGO!Soft Comfort**: der LOGO! 8 eine feste IP-Adresse im selben
   Netz wie der Pi geben, und den **Modbus-TCP-Server** im Programm
   aktivieren.
2. Die gewünschten Werte (Auslöse-Merker, ggf. ein Analogwert) auf
   Modbus-Register legen — die genauen Adressen stehen im
   LOGO!Soft-Comfort-Handbuch, Kapitel "Modbus TCP".
3. Auf dem Pi das Zusatzpaket installieren (bereits in `requirements.txt`
   enthalten):
   ```
   pip install pymodbus
   ```
4. Das mitgelieferte Skript `logo8_modbus_bridge.py` starten (IP-Adresse
   und Register-Nummern anpassen):
   ```
   python3 logo8_modbus_bridge.py --host 192.168.1.50 --trigger-reg 0 --value-reg 1
   ```
   Es verbindet sich mit der LOGO! 8, fragt laufend das Auslöse-Register ab
   und meldet jedes erkannte Ereignis.
